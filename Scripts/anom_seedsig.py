...
3. GOAL: automate gridding and axes limits
4. GOAL: Segmentation??
5. GOAL: automate parameter selection
'''
...
def loadCaseConfig(caseName): 
    ...
    case_configs = {
        "KPDT20260124": {
            "grid_limits": ((0, 2500), (-70000, 10000), (-70000, 10000)),
        ...
        "KPDT20260220_SM1_3": {
            "grid_limits": ((1300, 1700), (-10000, 15000), (20000, 100000)),
            "grid_shape": (4, 63, 200),
            "zslice": (0, 4),
            "max_velocity": 15.0,
            "min_distance": 5000.0,
        ...
        "KPDT20260220_SM4_13": {
            "grid_limits": ((1300, 1700), (2500, 27500), (20000, 100000)),
            "grid_shape": (4, 63, 200),
            "zslice": (0, 4),
            "max_velocity": 15.0,
            "min_distance": 5000.0,
...
def loadGridRadar(filenames, config):

    # extract config params
    grid_limits = config["grid_limits"]
    grid_shape = config["grid_shape"]
    zslice = config["zslice"]
...
def filterTracks(stats, tracks, config, radar, ymdt):
    ...
    #TODO call buildCone and applyCone here, then do the below good_cells line

    good_cells_cone, cartesian_drone_coords = buildCone(config, radar, tracks)
...
def buildCone(config, radar, tracks):
...
def animateTracks(da_track, tracks_filtered, cartesian_drone_coords, ymdt):
...
def main():
    ...
    filenames, case = selectCase()

    config = loadCaseConfig(case)

    da_track, dxy, radar = loadGridRadar(filenames, config)
    ...
    tracks_filtered, cartesian_drone_coords = filterTracks(stats, tracks, config, radar, ymdt)

    animateTracks(da_track, tracks_filtered, cartesian_drone_coords, ymdt)
