...
3. TODO: automate gridding and axes limits
4. TODO: Segmentation??
5. TODO: automate parameter selection
'''
...
def autoGridAxes(filenames, wind_location, radar, config): # TODO needs vector start and mathematical maximum extent of the plume --> drone location, wind vector, duration of radar loop
    ...
    print(f"target grid spacings = dx = {dx} m, dy = {dy} m, dz = {dz} m")
    print(f"actual grid spacings = dx = {actual_dx} m, dy = {actual_dy} m, dz = {actual_dz} m")
...
def loadGridRadar(filenames, config, box):
    ...
    #scans = pd.to_datetime(scans, utc=True)
    ...
    da_track = da.isel(z=slice(zslice[0], zslice[1])).max(dim="z")
...
def detectFeatures(da_track, dxy, stats, config, ymdt):
    ...
    n_erosion_threshold = config["n_erosion_threshold"]
    n_min_threshold = config["n_min_threshold"]
...
def filterTracks(stats, tracks, config, radar, ymdt, wind_location):
    ...
    return tracks_filtered, cartesian_drone_coords, good_cells
...
def animateTracks(da_track, tracks_filtered, cartesian_drone_coords, ymdt):#, segmentation_mask):
    ...
    #     alpha=0.35,
    #     vmin=0,
    #     vmax=int(segmentation_mask.max())
    # )
    ...
    def update(frame): # nested function to update the plot for each frame
        ...
        # seg.set_data(segmentation_mask.isel(time=frame))
...
def segmentation(da_track, tracks_filtered, dxy, good_cells):
    # only keep features that are in the good cells list
    tracks_segment = tracks_filtered[tracks_filtered["cell"].isin(good_cells)].copy()

    tracks_segment["time"] = (
        pd.to_datetime(tracks_segment["time"])
        .dt.tz_localize(None)
    )

    da_track = da_track.assign_coords(
        time=pd.to_datetime(da_track.time.values).tz_localize(None)
    )

    print(tracks_segment["time"].dtype)
    print(pd.to_datetime(tracks_segment["time"]).dt.tz)

    print(da_track.time.dtype)
    print(pd.to_datetime(da_track.time.values).tz)
...
def main():
    ...
    tracks_filtered, cartesian_drone_coords, good_cells = filterTracks(stats, tracks, config, radar, ymdt, wind_location)

    #segmentation_mask, features_mask = segmentation(da_track, tracks_filtered, dxy, good_cells)

    animateTracks(da_track, tracks_filtered, cartesian_drone_coords, ymdt)#, segmentation_mask)
